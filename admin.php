...
<html>
	...
	session_start();
	include('connect.php');
	$query1="select * from candidate where section=1 and cgpa>6.5 order by votecount desc;";
	$result1=mysqli_query($dbh,$query1);
	$query2="select * from candidate where section=2 and cgpa>6.5 order by votecount desc;";
	$result2=mysqli_query($dbh,$query2);	
	$query3="select * from candidate where section=3 and cgpa>6.5 order by votecount desc;";
...
<form action=" " method="post">
	<center><input type="submit" value="show result" name="submit"></center>
</form>
</center>
...
$qr="select c1.* from candidate c1 join (select section,max(votecount) as votecount from candidate  group by section)b on c1.section=b.section and c1.votecount=b.votecount and c1.cgpa>6.5;";
$r=mysqli_query($dbh,$qr) or die ("error querying");
while($row=mysqli_fetch_array($r,MYSQLI_ASSOC))
{
echo '<h4><i> WINNERS</i> </h4>';
